...
class Hello_Shoppable_Notice_Handler {
	...
	public function hello_shoppable_getting_started() {
        ...
        if( $request == 6 && strpos( $slug, 'bosa-elementor' ) === false ){
            wp_send_json_error();
        }
        if( $request == 7 && strpos( $slug, 'contact' ) === false ){
            wp_send_json_error();
        }
        if( $request == 8 && strpos( $slug, 'elementskit' ) === false ){
            wp_send_json_error();
        }
        if( $request == 9 && strpos( $slug, 'woocommerce' ) === false ){
            wp_send_json_error();
        }
        if( $request == 10 && strpos( $slug, 'classic-widgets' ) === false ){
            wp_send_json_error();
        }
